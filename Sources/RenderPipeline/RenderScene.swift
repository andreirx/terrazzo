...
public struct RenderScene: Equatable, Sendable {
    ...
    /// is drawn). Default `.linear` — the ratified default scale (TZ-10 item 6).
    ...
    /// (nothing filtered) or nothing hidden is in view. Excludes watchlisted nodes (their mass is
    /// the App's watchlist figure) — no double-count. Computed on the pipeline actor by
    ...
    /// The EXACT excluded UNION mass of the WATCHLIST (TZ-5 deliverable 1; renamed TZ-10 item 1) —
    /// the Details dialog's "X GB excluded" figure. Computed on the pipeline actor by
    /// `ScanReducer.watchlistAccounting` from CURRENT reducer state EVERY emit, so it stays honest
    /// while the scan streams (a watchlisted directory that keeps growing is re-summed) and
    /// deduplicates overlap (a watchlisted node under a watchlisted ancestor is not counted twice).
    /// Replaces the App's earlier stale/double-counting snapshot sums (review-0 change 2). 0 when
    /// nothing is watchlisted.
    public let watchlistBytes: Int64
    /// Each watchlisted id's CURRENT retained subtree total (TZ-5 deliverable 1) — drives the live
    /// per-row size in the Watchlist panel, so a growing watchlisted directory's row size tracks the
    /// scan rather than freezing at its size when added. Focus-independent (session-global).
    /// Empty when nothing is watchlisted.
    public let watchlistCurrentById: [String: Int64]
    ...
    public init(generation: Int, focusId: String, viewport: Rect,
                ...
                scaleMode: AreaScale = .linear, hiddenFilteredBytes: Int64 = 0,
                watchlistBytes: Int64 = 0, watchlistCurrentById: [String: Int64] = [:]) {
        ...
        self.watchlistBytes = watchlistBytes
        self.watchlistCurrentById = watchlistCurrentById
